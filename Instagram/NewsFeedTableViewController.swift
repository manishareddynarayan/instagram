...
class NewsFeedTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        tableView.register(UINib(nibName: "newsFeedTableViewCell", bundle: nil), forCellReuseIdentifier: "Cell1")
        tableView.rowHeight = 270.0
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "Cell1", for: indexPath) as! newsFeedTableViewCell
        imageFiles[indexPath.row].getDataInBackground { (data, error) in
            if let imageData = data {
                if let imageToDispaly = UIImage(data: imageData) {
                cell.postedImage.image = imageToDispaly
                }
            }
